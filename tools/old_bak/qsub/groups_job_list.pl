...
my %Queue=();### .. 
my %Job=();
my %For_sort=();### ..sort ..... sort ... $For_sort{$name}=info for sort;

my $jobs='';
foreach my $job ( @task ) {
	$job=~s/^\s+//;
	my @tab=split /\s+/,$job;#### $tab[0] = job id ; $tab[3]= ... ; $tab[4] = .. ; $tab[7] = .. ;
	$statue{$tab[4]}=1;
	...
	######
	$jobs.=$tab[0].',';
	if (exists $out{$tab[3]}{'status'}{$tab[4]} ) { ########.. ..
	...
	if ($tab[7]=~/(\S+)\@/) {
		...
		if (exists $out{$tab[3]}{'queue'}{$queue}) {########## .... ....
...
my $overload=0;
if (defined $mem) {
	my $detail=`qstat -j $jobs`;###... .. ....
	my @jobinfos=split /={2,}/,$detail;####.......
