...
my ( $total_width,$total_height,$total_height2 ) = (400,200,20);
my ($font_size0,$font_len0,$font0,$font_type0) = (15,5,"Arial","bold");
my ($font_size1,$font_len1,$font1,$font_type1) = (15,5,"Arail","normal");
my ($font_size2,$font_len2,$font2,$font_type2) = (11,5,"Arial","normal");
my ($font_size3,$font_len3,$font3,$font_type3) = (10,5,"Arial","normal");
...
#plotting
&plot_mut_onGene("deafness.1.svg","chr13","GJB2","NM_004004","rs80338942","Deletion","Leu56Argfs",20763554,20763554);
#NM_004004.5:c.167delT,NC_000013.10:g.20763554del

#ending
print "End   $0 time:".localtime(time)."\n";
sub plot_mut_onGene{
	my ( $svg_file,$chr,$gene_name,$nm_id,$rs_id,$mut_type,$protein_change,$mut_pos1,$mut_pos2 ) = @_;
	#save STDOUT handle
	...
	#&svg_begin();
	&svg_rect( 0,0,0,0,$total_width,$total_height,"white","black",1);
	&plot_chr($chr);
	&plot_gene($nm_id,$chr,$gene_name);
	&plot_mutation($nm_id,$mut_pos1,$mut_pos2,$mut_type,"c.167delT",$protein_change);
	&title("突变 : $rs_id");
	&svg_end();
	close STDOUT;
...
sub title{
	my ( $title_text ) = @_;
	my $scale = 3.5;
	&svg_text_rotate($total_width/2-length($title_text)*$font_len0/2,$total_height/10,"dark",$title_text,$font0,$font_size0,"0 0,0",$font_type0);
}
...
sub get_gene_pos{
	...
	my $y_start = (3/30+2/5)*$total_height;
	my $scale = (6*$total_width/10)/$h_mRNA_attr{$nm_id}{"len"};
...
sub plot_mutation{
	my ($back_col,$back_stroke_col,$back_stroke_size,$back_opacity) = ("green","green",0,0.4);
	my ($nm_id,$pos1,$pos2,$mut_type,$mut_dna,$protein_change) = @_;
	my ($gene_start_x,$gene_start_y,$gene_width,$gene_height) = &get_gene_pos($nm_id,$h_mRNA_attr{$nm_id}{"start"},$h_mRNA_attr{$nm_id}{"end"});
	my ($mut_start_x,$mut_start_y,$mut_width,$mut_height) = ( $gene_start_x+2*$gene_height,$gene_start_y+3*$gene_height,$gene_width,6.5*$gene_height);
	&svg_rect_with_opacity( $mut_start_x,$mut_start_y,5,5,$mut_width,$mut_height,$back_col,$back_stroke_col,$back_stroke_size,$back_opacity);
	#my ($title_text_x ,$title_text_y) = ($mut_start_x,$mut_start_y);
	#my $mut_font_size = 14;
	&svg_text_rotate($mut_start_x+2*$gene_height,$mut_start_y+2*$gene_height,"dark","突变类型：".$mut_type,$font2,$font_size2,"0 0,0",$font_type2);
	&svg_text_rotate($mut_start_x+2*$gene_height,$mut_start_y+3.8*$gene_height,"dark","碱基变化：".$mut_dna,$font2,$font_size2,"0 0,0",$font_type2);
	&svg_text_rotate($mut_start_x+2*$gene_height,$mut_start_y+5.6*$gene_height,"dark","蛋白影响：".$protein_change,$font2,$font_size2,"0 0,0",$font_type2);
	#$pos = 20767110;
	#$pos = 20761612;
	my ($x,$y,$width,$height) = &get_gene_pos($nm_id,$pos1,$pos2);
	my ($x1,$y1) = ($x-$gene_height,$mut_start_y);
	$x1 = ($mut_start_x+2*$gene_height) if( $x1 < ($mut_start_x+2*$gene_height) );
	$x1 = ($mut_start_x+$mut_width-3*$gene_height) if( $x1 > ($mut_start_x+$mut_width-3*$gene_height) );
	my ($x2,$y2) = ($x,$y+$height);
	my ($x3,$y3) = ($x1+$gene_height,$y1);
	&svg_polygon("$x1,$y1 $x2,$y2 $x3,$y3",$back_col,$back_stroke_col,$back_stroke_size,$back_opacity);
	&svg_ellipse($x2,$y+$height/2,2,$height-2,"red","red",0,0.5);
}
sub plot_gene{
	my ($back_col,$back_stroke_col,$back_stroke_size,$back_opacity) = ("red","red",0,0.3);
	...
	&svg_rect_with_opacity( $chr_x-10,$chr_y-4,3,3,$chr_width+20,$chr_height+8,"red",$back_stroke_col,$back_stroke_size,0.4);
	...
	#plot gene background color
	&svg_rect_with_opacity( 4*$x/5,$y-6.5*$height,3,3,$width+2*$x/5,8.5*$height,$back_col,$back_stroke_col,$back_stroke_size,$back_opacity);
	my ($x1,$y1) = (4*$x/5,$y-5.5*$height);
	my ($x2,$y2) = ($chr_x+$chr_width+5,$chr_y+$chr_height);
	my ($x3,$y3) = ($chr_x+$chr_width+5,$chr_y+$chr_height);
	my ($x4,$y4) = (4*$x/5,$y-3.5*$height);
	&svg_polygon("$x1,$y1 $x2,$y2 $x3,$y3 $x4,$y4",$back_col,$back_stroke_col,$back_stroke_size,$back_opacity);
	#plot gene pos
	&svg_rect( $x,$y-$height,$rx,$ry,$width,1,"black",$stroke_col,0);
	#gene start pos
	&svg_rect( $x,$y-3*$height/2,$rx,$ry,1,$height/2,"black",$stroke_col,0);
	&svg_text(9*$x/10,$y-3*$height/2-8,"black","From: ".&per_thousand_sign($h_mRNA_attr{$nm_id}{"start"}),$font_size3);
	#gene end pos
	&svg_rect( $x+$width-1,$y-3*$height/2,$rx,$ry,1,$height/2,"black",$stroke_col,0);
	my $to_pos = "To: ".&per_thousand_sign($h_mRNA_attr{$nm_id}{"end"});
	my $arial_8_scale = (300/50);
	&svg_text($x+$width-$font_len3*length($to_pos),$y-3*$height/2-8,"black","$to_pos",$font_size3);
	#title
	#&svg_text($x+$width/2-20,$y-3*$height/2-28,"rgb(139,69,19)",$nm_id,20);
	my $text_gene = "基因: ".$gene_name;
	my ($text_x,$text_y) = ($x+$width/2-length($text_gene)/2*$font_len1,$y-4.4*$height);
	&svg_text_rotate($text_x,$text_y,"dark",$text_gene,$font1,$font_size1,"0 0,0",$font_type1);
...
sub plot_chr{
	...
	$text_chr =~ s/^(\d+)([^\d])/$1 号/;
	my $text_chr_len = length($text_chr)/2;
	#$text_chr =~ s/chr/Chromosome /;
	my ($text_x,$text_y) = ($x/2,$y+$height/2+$text_chr_len*$font_len1);
	#my ($text_x,$text_y) = ($x-20,$y+$height/2+25);
	&svg_text_rotate($text_x,$text_y,"dark",$text_chr,$font1,$font_size1,"270 $text_x,$text_y",$font_type1);
