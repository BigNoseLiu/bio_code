...
def stat_common(df_sample, workdir, target_pathogen):
    ...
    df_merge_normal = pd.DataFrame()     # 以均一化结果
    spe2kingdom = {}
    spe2species_cn = {}
...
def fill_common(df_data, spe2comment):
    """ 补充共有检出统计 """
    if not spe2comment:
        return df_data

    # comment = []
    # for row in df_data[['SpeciesSN', 'SpeciesReads']].itertuples():
    #     comment.append('-'.join([spe2comment.get(row[1], '0'), str(row[2])]))
...
def fill(file_path, normalize, qc_file, cov_file, bg_info, nc_spe2rc, spe2comment):
    """ 补充背景菌、覆盖度、阴控数据 """
    ...
    if qc_file.exists():
        ...
        print(f'{qc_file} is not exists, please check it.')

    # Fill background species stat
    if len(bg_info):
        df_data = fill_background(df_data, bg_info)
    else:
        print(f'Background Stat Data is empty, please check it.')
    ...
    if len(spe2comment):
        df_data = fill_common(df_data, spe2comment)
    else:
        print('Common Stat Data is empty, please check it.')
    ...
    for file in 'Total_Detail.txt', 'Pathogeny_Detail.txt':
        for row in df_smp.itertuples():
            ...
            if not tax_file.exists():
                print(f'{tax_file} not exists, please check it.')
                continue
            ...
            pool_fill.apply_async(fill, args=(tax_file, normalize, qc_file, cov_file, sub_bg_info, nc_spe2rc, spe2comment))
    pool_fill.close()
    pool_fill.join()
