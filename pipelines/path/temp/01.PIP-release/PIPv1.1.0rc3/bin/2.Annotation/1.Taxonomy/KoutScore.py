...
def main(kout, top, db, out, kmercut):
        ...
        for _, line in enumerate(rfp):
            ...
            if kingdom == 'Viruses' and s1_tid:
                for mapping in ll[4].split(' '):
                    ...
                    if m[0] in seqid2s1_tid:
                        spe_kmer[seqid2s1_tid.get(m[0])] += int(m[1])
                    elif m[0] == s_tid:
                ...
                for mapping in ll[4].split(' '):
                    m = mapping.split(':')
                    total_kmer += int(m[1])
                    if m[0] in seqid2speid:
                        spe_kmer[seqid2speid.get(m[0])] += int(m[1])
                    elif m[0] == s_tid:
    ...
    with open(out, 'w') as wfp:
        ...
        if not df_viral.empty:
            for row in get_fail(df_viral, 's_tid', int(top), seqid2speid, spe_fail):
                wfp.write(row)
