...
`$BLASTN -query $outdir/validated.fna -db $BLASTDB/$ntV/nt -num_threads 20 -out $outdir/validated.m8 -outfmt \"6 qseqid sacc pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen qcovhsp\" -evalue 0.05 -word_size 28 -reward 1 -penalty -2 -perc_identity 80 -qcov_hsp_perc 20`; #-num_alignments 100000
...
####################################################################
#对于烟曲霉，kraken仅分类到烟曲霉唯一物种且Blast为空的，设置为TRUE  (以下存储%uniqspe的脚本来自KoutScore.pl)-hy 20220516-start
###################################################################
	...
	my %uniq_info; #hy
	my $kout=$inMark; $kout=~s/.MarkNum$//; 
	open IN,"$kout" or die $!;
	open TMP,">$outdir/readname_spenum" or die $!;#hy
	#my %sort;
	while(<IN>){
		next if(/^U/);
		chomp;
		...
		if(exists $seqid2speid{$t[2]}){
			...
			my $spenum=scalar(keys %uniqspe);print TMP "$t[1]\t$spenum\n";$uniq_info{$t[1]}=$spenum;		
		}
	}
	close IN;
	close TMP; #hy
...
####################################################################
#对于烟曲霉，kraken仅分类到烟曲霉唯一物种且Blast为空的，设置为TRUE  -hy 20220516-end
###################################################################
...
while(<IN>){
	...
	#print VO "$id\t$anno{$queryTaxID}\tfalse\n" if(!exists $score{$id} && exists $anno{$queryTaxID}); #hy -20220516
	
	#hy 20220516-start
	if(!exists $score{$id} && exists $anno{$queryTaxID}){
		my $readname0 = (split /\|/,$id)[0];
		my $spetaxid0="";
		if(exists $seqid2speid{$queryTaxID}){$spetaxid0=$seqid2speid{$queryTaxID};}
		if(($queryTaxID eq 746128) or ($spetaxid0 eq 746128)){#烟曲霉
			my $spenum="";
			if(exists $uniq_info{$readname0}){
				$spenum=$uniq_info{$readname0};
				if($spenum eq 1){ #kraken仅分类到烟曲霉一个物种
					print VO "$id\t$anno{$queryTaxID}\ttrue\n" ;
				}
			}
