...
def generateScript(L, outdir):
	...
	with open(L) as inf, open(sList, 'w') as sl, open(SH, 'w') as sh:
		for l in inf.readlines():
			...
			if Type == 'SE':
				sh.write('%(python)s %(resfinder)s --cpu %(thread)s -db_res_kma %(resfinderDB)s -s Other '
					...
					'coverage': args['threshold'], 'identity': args['identity'], 'samplePath': samplePath, 'python': cfg.python})
			else:
				sh.write('%(python)s %(resfinder)s --cpu %(thread)s -db_res_kma %(resfinderDB)s -s Other '
