...
def check_fastp(fastp_dir, sample, library):
    """ 检查质控结果文件是否齐全并返回信息 """
    gz_file = Paht(fastp_dir) / sample / library / f'{sample}.fq.gz'
...
def check_tax(tax_dir, sample, library):
    """ 检查物种鉴定结果文件是否齐全并返回信息 """
    file_path = Path(tax_dir) / sample / library / 'Total_Detail.xlsx'
    if not file_path.exists():
        ...
        if not df.empty:
            gz_exists = False
            for gz in file_path.parent.glob('*result.fq.gz'):
                if gz.exists():
                    gz_exists = True
            if gz_exists:
                return 'Info', ''
...
def check_result(sample, smp_dir, workdir):
    ...
    file_lst = ['QC.Stat.xlsx', 'raw_summary.xls', 'Taxonomy_Summary.txt',
                'ARG.cov.pick.xls', 'VF.xls', 'Pathogeny_Detail.txt', 'Pathogeny_Detail.xlsx',
...
def custom_dict():
    return {
        '00.Merge': {'comment': '数据合并', 'note': '分析中', 'label': 'Info', 'log': ''},
        '01.Fastp': {'comment': 'QC质控', 'note': '分析中', 'label': 'Info', 'log': ''},
        '02.RemoveHost': {'comment': '去宿主', 'note': '分析中', 'label': 'Info', 'log': ''},
        '03.Taxonomy-Classify': {'comment': '物种鉴定-比对统计', 'note': '分析中', 'label': 'Info', 'log': ''},
        '04.Taxonomy-Coverage': {'comment': '物种鉴定-覆盖度', 'note': '分析中', 'label': 'Info', 'log': ''},
        '05.ResistantGene': {'comment': '耐药检测', 'note': '分析中', 'label': 'Info', 'log': ''},
        '06.VF': {'comment': '毒力分析', 'note': '分析中', 'label': 'Info', 'log': ''},
        '07.Stat': {'comment': '批次统计', 'note': '分析中', 'label': 'Info', 'log': ''}
    }


def workflow(samples, workdir):
    """ 检查各个步骤的分析状态，反馈分析进度 """
    ...
    for arg_log in (arg_dir / 'shell').glob('ResistanceGene.sh.*.log'):
        if arg_log.exists():
            ...
            for task, smp, note, label, log_text in get_note(task2status, smp2lib):
                ...
                if note == '分析完成':
                    arg_res_file = arg_dir / smp / smp2lib[smp] / 'ARG.cov.pick.xls'
    ...
    for vf_log in (vf_dir / 'shell').glob('VFDB.sh.*.log'):
        if vf_log.exists():
            ...
            for task, smp, note, label, log_text in get_note(task2status, smp2lib):
                ...
                if note == '分析完成':
                    vf_res_file = vf_dir / smp / smp2lib[smp] / 'VF.xls'
    ...
    complete_tag = True
    if len(failed_smp) != len(smp2lib):
        for smp, stat in result['sampleSchedule'].items():
            for ana in stat.keys():
                if ana == '07.Stat':
                    continue
                if stat[ana]['note'] == '分析中':
                    complete_tag = False

    time.sleep(10)

    done2file = Path(workdir) / 'by2.done'

    if complete_tag and done2file.exists():
        for smp in smp2lib.keys():
            smp_dir = res_dir / smp
    ...
    data = {time_tag: result}
    with open(workdir / 'shell/pipeline.log', 'a') as wfp, open(workdir / 'shell/pipeline.log.json', 'a') as wfp2:
        json.dump(data, wfp, ensure_ascii=False)
        wfp.write('\n')
        json.dump(data, wfp2, ensure_ascii=False, indent=4)
    print(time_tag, complete_tag)
