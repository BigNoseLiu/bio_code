		...
		for l in inf.readlines():
			...
			if Type == 'SE':
				...
				sh.write('%(python39)s %(resfinder)s --cpu %(thread)s -db_res_kma %(resfinderDB)s -s Other '
					...
					'coverage': args['threshold'], 'identity': args['identity'], 'samplePath': samplePath, 'python39': cfg.python39})
			sh.write('rm -fr %(samplePath)s/resfinder_kma &\n'
					 'wait\n' % \
...
#		exit(0)
	...
	if mode.upper() == 'SLURM':
		os.system(f'{cfg.perl} {cfg.sbatch} --maxjob 51 --cpu {args["thread"]} --lines 3 --jobprefix ResistanceGene {cmd}')
	else:
		os.system(f'{cfg.perl} {cfg.watchDog} --mem 2G --num_paral 17 --num_line 3 {cmd}')
