...
def get_tax_info(db, sample_type):
    ...
    for file in ['all.Taxonomy.txt', 'all.Taxonomy.other.txt']:
        ...
        for _, line in enumerate(fp):
            ...
            if line:
                ll = line.split('\t')
                tax_infos['tax_id2class'][ll[0]] = ll[2]
                tax_infos['tax_id2class'][ll[1]] = ll[2]
...
def get_group_reads(kreport, nodelete_groups, group2infos):
        ...
        for _, line in enumerate(rfp):
            ...
            if line:
                ...
                if sci_name in group2infos:
                    if sci_name in nodelete_groups:
    ...
    for spe_lv in spe_level2stat.keys():
        if len(spe_level2stat[spe_lv]['types']) == 0:
            ...
            for strain in spe_level2stat[spe_lv]['types']:
                ...
                if (spe_uniqtotal * spe_common) == 0:
                    spe_level2rc[strain] = strain_rc
                else:
...
def main(report, mpa, db, name, validate, highconf, type, outdir, version):
    ...
    grp_tid_set = {group2infos[g]['GroupTaxid'] for g in group2infos.keys()}

    with open(report, 'r') as rfp:
        has_pathogen = False
        for _, line in enumerate(rfp):
            ll = line.strip().split('\t')
            if ll:
                if ll[4] in tax_infos['tax_id2class'] or ll[4] in grp_tid_set:
                    has_pathogen = True
                    break
        if not has_pathogen:
            ...
            xlsx_total = Path(outdir).joinpath('Total_Detail.xlsx')
            xlsx_pathogen = Path(outdir).joinpath('Pathogeny_Detail.xlsx')
            for file in taxid_r, file_total, file_pathogen, xlsx_total, xlsx_pathogen:
                os.system(f'touch {str(file)}')
            return
    ...
    group2reads_cnt = get_group_reads(report, validate_res['nodelete_groups'], group2infos)
    ...
    with open(file_total, 'w') as out_total, open(file_pathogen, 'w') as out_pathogen:
        ...
        for spe, rpm in tax_stat_res['spe2RPM'].items():
            ...
            if group:
                ...
                group_cn = '-'
            group_rc = str(group2reads_cnt.get(group, 0))
