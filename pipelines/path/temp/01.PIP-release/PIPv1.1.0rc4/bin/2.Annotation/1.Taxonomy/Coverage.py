...
def draw_plot(data, name, outfile):
    ...
    y3_max = max(y3_data)
    y2_ticks = [round(y3_max - y3_max * i, 1) for i in range(0, 5, 1)][::-1]
    ax2.set_ylim(y2_ticks[0], y2_ticks[-1])
    ax2.set_yticks(y2_ticks)
    ax2.tick_params(axis='y', colors='darkorange')
    for i, ytl in enumerate(ax2.get_yticklabels()):
        if i < 3:
    ...
    plt.gca().legend(handles=patches, loc='upper center', bbox_to_anchor=(0.5, 1.16), ncol=3, fontsize=9)
...
def stat4cov(species, spe_reads, genus, gns_reads, read_len, tax_info, acc_info, df_data, out_png):
    ...
    for i in range(bin_num):
        ...
        result['AvgDepth'][i + 1] = (uni_rc + mul_rc) * read_len / bin_len
    ...
    for kd, tag in KINGDOM2TAG.items():
        ...
        for row in df_sub[['SpeciesSN', 'SpeciesReads', 'GenusSN']].itertuples():
            if ' complex' in row[1] or ' group' in row[1]:
                continue
            out_png = Path(indir) / f'Coverage/04.plot/{tag}' / (row[1].replace(' ', '_') + '.coverage.png')
