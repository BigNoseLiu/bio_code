...
def main(samples, analysis, workdir, thread):
    ...
    for row in df.itertuples():
        ...
        for file in ['Pathogeny_Detail', 'Total_Detail', 'Taxonomy_Summary']:
            pool.apply_async(cp, args=(tax_dir / row[1] / row[2] / f'{file}.*', smp_dir))
        # copy ARG results
        pool.apply_async(cp, args=(arg_dir / row[1] / row[2] / 'ARG.cov.pick.xls', smp_dir))
        # copy VF results
        pool.apply_async(cp, args=(vf_dir / row[1] / row[2] / 'VF.xls', smp_dir))
    ...
    for row in df.itertuples():
        smp_dir = result_dir / row[1]
        # copy sequences
        pool.apply_async(cp, args=(tax_dir / row[1] / row[2] / 'ResultSeq/*.result.fa.gz', smp_dir))
        # copy coverage plot
