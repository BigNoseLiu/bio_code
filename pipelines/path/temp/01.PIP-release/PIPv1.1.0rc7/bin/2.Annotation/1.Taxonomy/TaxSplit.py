...
def build(tax_info, outdir, db, thread):
    ...
    with open(out_seq, 'w') as wfp_seq, open(prelim_map, 'w') as wfp_map:
        for gz_file in Path(outdir).glob('*.result.fq.gz'):
            with gzip.open(gz_file, 'rb') as rfp:
                grp_header = False
                for line in rfp:
                    line_str = str(line, 'utf-8')
                    if line_str.startswith('>'):
                        seq_id = line_str.split(' ')[0].replace('>', '', 1)
                        if '|' in line_str:
                            ll = line_str.strip().split(' | ')
                            # to group/complex
                            grp_name = ' '.join(ll[0].split(' ')[1:])
                            grp_tid = tax_info['name2tid'].get(grp_name, '0')
                            grp_header = f'>kraken:taxid|{grp_tid}|{seq_id}\n'
                            wfp_map.write(f'TAXID\tkraken:taxid|{grp_tid}|{seq_id}\t{grp_tid}\n')
                            # to species
                            spe_tid = tax_info['name2tid'].get(ll[1], '0')
                            wfp_seq.write(f'>kraken:taxid|{spe_tid}|{seq_id}\n')
                            wfp_map.write(f'TAXID\tkraken:taxid|{spe_tid}|{seq_id}\t{spe_tid}\n')
                        else:
                            spe_tid = tax_info['name2tid'].get(' '.join(line_str.strip().split(' ')[1:]), '0')
                            wfp_seq.write(f'>kraken:taxid|{spe_tid}|{seq_id}\n')
                            wfp_map.write(f'TAXID\tkraken:taxid|{spe_tid}|{seq_id}\t{spe_tid}\n')
                    else:
                        wfp_seq.write(str(line, 'utf-8'))
                        if grp_header:
                            wfp_seq.write(grp_header)
                            wfp_seq.write(str(line, 'utf-8'))
                            grp_header = False
...
def main(kraken_out, result, db, fq, outdir, thread):
    ...
    build(tax_info, outdir, db, thread)
