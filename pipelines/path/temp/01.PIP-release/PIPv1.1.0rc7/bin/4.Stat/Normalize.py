...
def main(workdir, samples, n_million):
    ...
    for smp, lib in smp2lib.items():
        ...
        for file in total_file, patho_file:
            ...
            df['GenusReads(Normalization)'] = [math.ceil(i * coe) for i in df['GenusReads']]
            df['GroupReads(Normalization)'] = [math.ceil(i * coe) for i in df['GroupReads']]
            df['SpeciesReads(Normalization)'] = [math.ceil(i * coe) for i in df['SpeciesReads']]
