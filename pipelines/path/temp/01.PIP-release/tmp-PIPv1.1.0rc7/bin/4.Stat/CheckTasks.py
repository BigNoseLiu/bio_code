...
def write_data(data, file_path, indent):
    if file_path.exists():
        with open(file_path, 'r') as rfp:
            old_data = rfp.read()
        with open(file_path, 'w') as wfp:
            if indent:
                json.dump(data, wfp, ensure_ascii=False, indent=indent)
            else:
...
def main(samples, workdir):
    ...
    log_file = Path(workdir) / 'shell/pipeline.log'
    jsn_file = Path(workdir) / 'shell/pipeline.log.json'

    while True:
        done_file1 = Path(workdir) / 'by2stat.done'
        done_file2 = Path(workdir) / 'by2.done'
        if done_file1.exists() or done_file2.exists():
            break
        else:
            time_tag = time.strftime('%Y-%m-%d %H:%M:%S')
            res = workflow(samples, workdir, False)
            data = {time_tag: res}
            write_data(data, log_file, 0)
            write_data(data, jsn_file, 4)
            time.sleep(60)
    ...
    if done_file1.exists():
        os.system(f'rm {str(done_file1)}')
