...
def check_file(path):
    if not Path(path).exists():
        print(f"Error: {str(path)} not exists! Please check it.")
...
def main(workdir, samples):
    ...
    for sample, library in sample2library.items():
        qc_stat[sample]['样本编号'] = sample
        # fastp QC
        jsn_file = qc_dir / f'{sample}/{library}/{sample}.json'
        if not jsn_file.exists():
            print(f"Error: {str(jsn_file)} not exists! Please check it.")
            continue
        fastp_info = json.load(jsn_file.open('r'))
        qc_stat[sample]['RawReads'] = fastp_info["summary"]["before_filtering"]["total_reads"]
        adapter_reads = fastp_info["adapter_cutting"]["adapter_trimmed_reads"]
        qc_stat[sample]['Adapter_ratio(%)'] = '{:.3f}'.format(
            get_quotient(adapter_reads, qc_stat[sample]['RawReads']) * 100, 3)
        qc_stat[sample]['Duplication(%)'] = '{:.3f}'.format(fastp_info["duplication"]["rate"] * 100, 3)
        qc_stat[sample]['CleanReads'] = fastp_info["summary"]["after_filtering"]["total_reads"]
        qc_stat[sample]['CleanQ30(%)'] = '{:.3f}'.format(fastp_info["summary"]["after_filtering"]["q30_rate"] * 100, 3)
        qc_stat[sample]['Effective(%)'] = '{:.3f}'.format(get_quotient(qc_stat[sample]['CleanReads'],
                                                                       qc_stat[sample]['RawReads']) * 100, 3)
        ...
        # Remove host stat
        rh_file = rh_dir / sample / library / 'coverage.report'
        if rh_file.exists():
            with open(rh_file, 'r') as rfp:
                for line in rfp.readlines():
                    ll = line.strip().split('\t')
                    if len(ll) == 2:
                        if '[Total] Mapped Reads' in ll:
                            qc_stat[sample]['Host Reads'] = int(ll[1])
                            # print(qc_stat[sample]['Host Reads'], ll[1])
                        # if '[Target] Target Reads' in ll:
                        #     qc_stat[sample]['Internal Control Reads'] = int(ll[1])
                        #     qc_stat[sample]['Host Reads'] -= qc_stat[sample]['Internal Control Reads']
                            # qc_stat[sample]['Internal Control Rate (%)'] = get_quotient(
                            #     qc_stat[sample]['Internal Control Reads'], qc_stat[sample]['Clean Reads']) * 100
        else:
            print(f"Error: {str(rh_file)} not exists! Please check it.")
        ...
        # else:
        #     print(f"Error: {str(rg_file)} not exists! Please check it.")

        # Taxonomy stat

        ts_file = tx_dir / sample / library / 'Taxonomy_Summary.txt'
        if not ts_file.exists():
            print(f'Error: {str(ts_file)} not exists, please check it.')
        else:
            df = pd.read_csv(ts_file, sep='\t', dtype=str)
            clean_reads = qc_stat[sample]['CleanReads']
            # qc_stat[sample]['Classified_Reads_Number'] = int(df.loc[0, 'Classified_Reads_Number']) - int(df.loc[0, 'Human'])
            # qc_stat[sample]['Classified_Rate (%)'] = get_quotient(qc_stat[sample]['Classified_Reads_Number'], clean_reads) * 100
            # qc_stat[sample]['Unclassified_Reads_Number'] = int(df.loc[0, 'Unclassified_Reads_Number'])
            # qc_stat[sample]['Unclassified_Rate (%)'] = get_quotient(qc_stat[sample]['Unclassified_Reads_Number'], clean_reads) * 100
            qc_stat[sample]['Host Reads'] += int(df.loc[0, 'Human'])
            qc_stat[sample]['人源比例'] = '{:.3f}'.format(get_quotient(qc_stat[sample]['Host Reads'], clean_reads) * 100, 3)
            # qc_stat[sample]['Viruses_Reads_Number'] = int(df.loc[0, 'Viruses'])
            # qc_stat[sample]['Viruses_Rate (%)'] = get_quotient(qc_stat[sample]['Viruses_Reads_Number'], clean_reads) * 100
            # qc_stat[sample]['Bacteria_Reads_Number'] = int(df.loc[0, 'Bacteria'])
            # qc_stat[sample]['Bacteria_Rate (%)'] = get_quotient(qc_stat[sample]['Bacteria_Reads_Number'], clean_reads) * 100
            # qc_stat[sample]['Fungi_Reads_Number'] = int(df.loc[0, 'Fungi'])
            # qc_stat[sample]['Fungi_Rate (%)'] = get_quotient(qc_stat[sample]['Fungi_Reads_Number'], clean_reads) * 100
            # qc_stat[sample]['Protozoa_Reads_Number'] = int(df.loc[0, 'Protozoa'])
            # qc_stat[sample]['Protozoa_Rate (%)'] = get_quotient(qc_stat[sample]['Protozoa_Reads_Number'], clean_reads) * 100
            # qc_stat[sample]['Metazoa_Parasite_Reads_Number'] = int(df.loc[0, 'Metazoa_Parasite'])
            # qc_stat[sample]['Metazoa_Parasite_Rate (%)'] = get_quotient(qc_stat[sample]['Metazoa_Parasite_Reads_Number'], clean_reads) * 100

        # get microbe reads
        tt_file = tx_dir / sample / library / 'Total_Detail.raw.txt'
        if tt_file.exists():
            df_tt = pd.read_csv(tt_file, sep='\t')
            if not df_tt.empty:
                qc_stat[sample]['微生物reads'] = sum(df_tt['SpeciesReads'])
                qc_stat[sample]['微生物比例'] = '{:.3f}'.format(get_quotient(qc_stat[sample]['微生物reads'],
                                                                        qc_stat[sample]['CleanReads']) * 100, 3)
    # QC judge
    for sample in sample2library.keys():
        if qc_stat[sample]['RawReads'] >= 20 * 10 ** 6 and float(qc_stat[sample]['CleanQ30(%)']) >= 90 and \
                float(qc_stat[sample]['Adapter_ratio(%)']) <= 10 and float(qc_stat[sample]['Effective(%)']) >= 70 and \
                qc_stat[sample]['微生物reads'] >= 1000:
            qc_stat[sample]['QC'] = 'Pass'
        else:
            qc_stat[sample]['QC'] = 'Fail'
        del qc_stat[sample]['Host Reads']

    df_out = pd.DataFrame().from_dict(qc_stat)
    for col in df_out.columns:
        df_out[col] = [reformat(i) for i in df_out[col]]
    ...
    batch_qc = {'Number of samples': len(sample2library)}
    ...
    if smr_tbl.exists():
        ...
        batch_qc['Effective (%)'] = get_quotient(batch_qc['MaxCleanReads'], batch_qc['TotalRawReads']) * 100
    else:
        batch_qc['ChipProductivity (%)'] = ''
        batch_qc['SplitRate (%)'] = ''
        batch_qc['Raw Q30 (%)'] = ''
        batch_qc['TotalRawReads'] = ''
        batch_qc['MaxRawReads'] = ''
        batch_qc['MinRawReads'] = ''
        batch_qc['MeanRawReads'] = ''
        batch_qc['TotalCleanReads'] = ''
        batch_qc['MaxCleanReads'] = ''
        batch_qc['MinCleanReads'] = ''
        batch_qc['MeanCleanReads'] = ''
        batch_qc['Clean Q30 (%)'] = ''
        batch_qc['Effective (%)'] = ''
    batch_qc['Number of QC-PASS samples'] = 0
    batch_qc['Number of QC-FAIL samples'] = 0
    for sample in sample2library.keys():
        if qc_stat[sample]['QC'] == 'Pass':
            batch_qc['Number of QC-PASS samples'] += 1
        elif qc_stat[sample]['QC'] == 'Fail':
            batch_qc['Number of QC-FAIL samples'] += 1
