...
def get_qc_df(in_dir, sample, library, rh_reads):
    ...
    if not fastp_smry.exists():
        return pd.DataFrame(), {'Num reads': 0, 'Num Bases': 0}
    ...
    qc_stat = Path(in_dir) / f'plugin_out/Result/{sample}/QC.Stat'
    df = pd.read_csv(qc_stat, sep='\t', dtype=str)
...
def workflow(workdir, name, library):
    ...
    sheet_name2df['QC'], qc_stats = get_qc_df(workdir, name, library, rh_reads)
