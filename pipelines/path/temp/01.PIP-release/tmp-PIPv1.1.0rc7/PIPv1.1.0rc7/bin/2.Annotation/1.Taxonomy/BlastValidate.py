...
def main(mark, kout, db, nt, fq, outdir):
    ...
    print('Blast Elapsed Time (s): ', time.time() - s_time)
    ...
    mark_seqid = set()
    viral_spe_tid_set = set(tax_info['tid2spe_tid'].values())
    with open(vld_m8, 'r') as rfp:
        for _, line in enumerate(rfp):
            ll = line.strip().split('\t')
            ...
            if float(ll[2]) + float(ll[14]) == seqid2score.get(ll[0], '-'):
                if tax_info['tid2kingdom'].get(query_tid) == 'Viruses':
                    # 病毒种水平
                    if query_tid in viral_spe_tid_set:
                        # 种比对上亚种
                        if query_tid == tax_info['tid2spe_tid'].get(subject_tid):
                            mark_seqid.add(ll[0])
                    # 病毒亚种水平
                    else:
                        # 亚种 == 亚种； 亚种比对上种
                        if query_tid == subject_tid or tax_info['tid2spe_tid'].get(query_tid) == subject_tid:
                            mark_seqid.add(ll[0])
                else:
