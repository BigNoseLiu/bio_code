...
def stat4cov(species, spe_reads, genus, gns_reads, read_len, tax_info, acc_info, df_data, out_png):
    ...
    if df_kout.empty:
        print('################')
        print(f'\"{species}\" mapped nothing. SpeciesTaxid=', spe_tid)
...
def main(indir, db, thread, outdir):
    ...
    for kd, tag in KINGDOM2TAG.items():
        ...
        align(cov_db, target_tid, tag, align_dir, str(thread))
    ...
    for kd, tag in KINGDOM2TAG.items():
        ...
        if len(df_sub) > step_num:
            for i in range(int(len(df_sub) / step_num) + 1):
                ...
                kd2res[kd] = stat_pool.apply_async(LogExceptions(stat),
    ...
    spe2res = {}
    for kd, res in kd2res.items():
        spe2res.update(res.get())
