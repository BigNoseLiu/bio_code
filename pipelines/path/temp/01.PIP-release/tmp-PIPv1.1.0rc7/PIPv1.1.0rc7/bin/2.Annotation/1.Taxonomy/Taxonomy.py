...
def generateScripts(L, outdir):
	...
	with open(L) as inf, open(sList, 'w') as sL, open(SH, 'w') as sh, open(SH2, 'w') as sh2:
		for fullLine in inf.readlines():
			...
			# 		 '--db %(KrakenDB)s/%(dbVersion)s --fq %(fq)s --outdir %(samplePath)s -t 10\n ' % \
			...
			sh.write(f"{cfg.python} {TaxSplit} -k {samplePath}/{l[0]}.unique.kout -r {samplePath}/Total_Detail.txt "
					 f"--db {cfg.KrakenDB}/{args['db']} --fq {samplePath}/{l[0]}.kout.C.fastq --outdir {samplePath} "
					 f"-t 21\n")

			sh2.write(f'{cfg.python} {Coverage} -i {samplePath}  -o {samplePath} --db {cfg.KrakenDB}/{dbVersion} -t 21\n')
...
#		exit(0)
	...
	CovParallel = int(MaxThread / 20)
	CovMem = int(MaxThread / CovParallel)
	...
	if mode.upper() == 'SLURM':
		...
		os.system(f'{cfg.perl} {cfg.sbatch} --maxjob {CovParallel} --cpu {thread} --lines 1 --jobprefix Coverage {outdir}/shell/Coverage.sh')
	elif mode.upper() == 'SGE':
		os.system(f'{cfg.perl} {cfg.SGE} --resource vf={TaxMem}g,p={thread} --lines 10 --queue all.q --maxjob {num_paral} --jobprefix Taxonomy {cmd}')
		os.system(f'{cfg.perl} {cfg.SGE} --resource vf={CovMem}g,p={thread} --lines 1 --queue all.q --maxjob {CovParallel} --jobprefix Coverage {outdir}/shell/Coverage.sh')
	else:
		os.system(f'{cfg.perl} {cfg.watchDog} --mem {TaxMem}G --num_paral {num_paral} --num_line 10 {cmd}')
		os.system(f'{cfg.perl} {cfg.watchDog} --mem {CovMem}G --num_paral {CovParallel} --num_line 1 {outdir}/shell/Coverage.sh')
