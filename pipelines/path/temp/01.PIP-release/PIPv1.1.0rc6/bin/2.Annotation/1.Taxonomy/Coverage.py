...
def align(cov_db, target_tid, kingdom_tag, out_dir, thread):
    ...
    for tid in target_tid:
        spe_fna = Path(cfg.coverage) / 'v1.0.1' / kingdom_tag / f'{tid}.fna'
        if spe_fna.exists():
...
def main(indir, db, thread, outdir):
    ...
    for kd, tag in KINGDOM2TAG.items():
        ...
        alg_pool.apply_async(LogExceptions(align), args=(cov_db, target_tid, tag, align_dir, str(thread)))
    alg_pool.close()
    alg_pool.join()
    ...
    for kd, tag in KINGDOM2TAG.items():
        ...
        spe_tid_lst = []
        for gnm_id in df_kout[1]:
            tid = gnm_id.split('|')[1]
            if tid in tax_info['grp_tid_set']:
                spe_tid_lst.append(tid)
            else:
                spe_tid_lst.append(tax_info['tid2spe_tid'].get(tid))
        df_kout['SpeTaxid'] = spe_tid_lst
