...
use MyModule::GlobalVar qw($PIGZ $BWA $SAMTOOLS);
...
my($fqs, $outf, $taxid, $krakenOut, $outdir, $db, $version);
GetOptions(
	"fq:s" => \$fqs,
	"outf:s" => \$outf,
	"taxid:s" => \$taxid,
...
&help unless($fqs && $taxid && $krakenOut && $db);
$outf ||= "fastq";
$outdir ||= "./Result";
...
if(scalar @FQs == 1){
	...
	if($outf eq 'fastq'){
		open SH,">$outdir/Align.sh" or die $!;
		for(@S){
			print "$outdir/$s2k{$_}/$_\n";
			...
			print SH "num=`$BWA mem -t 1 -Y -M $db/BWA/$refname/$refname.fna $outdir/$s2k{$_}/$_/$name.gz |$SAMTOOLS flagstat - |grep mapped |head -n1 |awk '{print \$1}'`;if [ \$num -eq 0 ]; then echo -e \"$s2k{$_}\\t$_\" > $outdir/$s2k{$_}/$_/unmapped;fi\n";
		}
		close SH;
	}elsif($outf eq 'fasta'){
		$name =~ s/\.fq|\.fastq//g;
	...
	if($outf eq 'fastq'){
		...
		for(@S){
			...
			print SH "num=`$BWA mem -t 1 -Y -M $db/BWA/$refname/$refname.fna $outdir/$s2k{$_}/$_/$name1.gz $outdir/$s2k{$_}/$_/$name2.gz |$SAMTOOLS flagstat - |grep mapped |head -n1 |awk '{print \$1}'`;if [ \$num -eq 0 ]; then echo \"$s2k{$_}\\t$_\" > $outdir/$s2k{$_}/$_/unmapped;fi\n";
		}
		close SH;
	}elsif($outf eq 'fasta'){
