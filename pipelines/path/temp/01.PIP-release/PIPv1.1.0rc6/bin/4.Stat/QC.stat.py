...
def main(workdir, samples):
    ...
    for sample, library in sample2library.items():
        ...
        if qc_stat[sample]['Raw Reads (M)'] >= 20:
            qc_pass += 1
        else:
            qc_fail += 1
        ...
        if not ts_file.exists():
            ...
            # qc_stat[sample]['Classified_Reads_Number'] = '-'
            # qc_stat[sample]['Classified_Rate (%)'] = get_quotient(qc_stat[sample]['Classified_Reads_Number'],
            #                                                       clean_reads) * 100
            # qc_stat[sample]['Unclassified_Reads_Number'] = '-'
            # qc_stat[sample]['Unclassified_Rate (%)'] = get_quotient(qc_stat[sample]['Unclassified_Reads_Number'],
            #                                                         clean_reads) * 100
            # qc_stat[sample]['Host Reads'] += 0
            # qc_stat[sample]['Host Rate (%)'] = round(get_quotient(qc_stat[sample]['Host Reads'], clean_reads) * 100, 3)
            # qc_stat[sample]['Viruses_Reads_Number'] = '-'
            # qc_stat[sample]['Viruses_Rate (%)'] = '-'
            # qc_stat[sample]['Bacteria_Reads_Number'] = '-'
            # qc_stat[sample]['Bacteria_Rate (%)'] = '-'
            # qc_stat[sample]['Fungi_Reads_Number'] = '-'
            # qc_stat[sample]['Fungi_Rate (%)'] = '-'
            # qc_stat[sample]['Protozoa_Reads_Number'] = '-'
            # qc_stat[sample]['Protozoa_Rate (%)'] = '-'
            # qc_stat[sample]['Metazoa_Parasite_Reads_Number'] = '-'
            # qc_stat[sample]['Metazoa_Parasite_Rate (%)'] = '-'
        else:
    ...
    if smr_tbl.exists():
        ...
        batch_qc['Clean Q30 (%)'] = ''
        batch_qc['Effective (%)'] = ''
    batch_qc['Number of QC-PASS samples'] = qc_pass
    batch_qc['Number of QC-FAIL samples'] = qc_fail
