...
@click.option('-t', '--thread', help='Specified how many threads can be used. [120]', default=120)
def main(samples, analysis, workdir, thread):
    ...
    for row in df.itertuples():
        smp_dir = result_dir / row[1]
        # link sequences
        seq_src_dir = tax_dir / row[1] / row[2] / '*.result.fq.gz'
        pool.apply_async(os.system, args=(f'ln -s {str(seq_src_dir)} {smp_dir}',))
        ...
        for kd_dir in plot_dir.glob('*'):
            ...
            # pool.apply_async(LogExceptions(cp), args=(kd_dir, result_dir / row[1] / f'{kd_dir.name}_readscount_coverage_plot'))
    pool.close()
    pool.join()
