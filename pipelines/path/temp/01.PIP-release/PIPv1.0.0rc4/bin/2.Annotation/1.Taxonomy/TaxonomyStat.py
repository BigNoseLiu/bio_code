...
TXT_HEADER = "Kingdom\tGenusName\tGenusReads\tGroupName\tGroupReads\tScientificName\t" \
             "ChineseName\tReads_Number\tGMRN\tTPM\tRH-RPM\tVerification\tFocus\n"
...
def main(report, mpa, db, name, validate, highconf, type, outdir, version):
    ...
    with open(file_total, 'w') as out_total, open(file_pathogen, 'w') as out_pathogen, \
        ...
        for spe, rpm in tax_stat_res['spe2RPM'].items():
            ...
            if superkingdom:
                ...
                if genus == 'NA' or not genus:
                    ...
                    out_total.write(f"\t{genus}\t{genus_rc}")
                if group:
                    out_total.write(f"\t{group}\t{group_rc}")
                else:
                    out_total.write("\t-\t0")
                out_total.write(f"\t{spe}\t{spe_cname}\t{spe_lv_rc}\t{genus_max_rc}\t{tpm}\t{rpm}")
                if check_tag:
                    out_total.write(f"\t{check_tag}\t{target_tag}\n")
                else:
                    if score:
                        out_total.write(f"\t{score}\t{target_tag}\n")
                    else:
                        out_total.write(f"\tFalse\t{target_tag}\n")

                if superkingdom != 'Viruses':
                    if not re.search(' sp\.| genomosp\.| genosp\.', spe):
                        out_pathogen.write(superkingdom)
                        if genus and genus != 'NA':
                            out_pathogen.write(f"\t{genus}\t{genus_rc}")
                        else:
                            out_pathogen.write(f"\t{genus}\t0")
                        if group:
                            out_pathogen.write(f"\t{group}\t{group_rc}")
                        else:
                            out_pathogen.write(f"\t-\t0")
                        out_pathogen.write(f"\t{spe}\t{spe_cname}\t{spe_lv_rc}\t{genus_max_rc}\t{tpm}\t{rpm}")
                        if check_tag:
                            out_pathogen.write(f"\t{check_tag}\t{target_tag}\n")
                        else:
                            if score:
                                out_pathogen.write(f"\t{score}\t{target_tag}\n")
                            else:
                                out_pathogen.write(f"\t-\t{target_tag}\n")
        ...
        for grp in group2reads_cnt.keys():
            if group2infos[grp]['Focus'] == '*':
                ...
                txt_row = [skd, gns, gns_rc, grp, grp_rc, grp, grp_cn, grp_rc, gns_max_rc, "-\t-\t-\t*\n"]
                ...
                xls_row = [str(grp_tid), grp, grp_rc, '-', grp_cn, gns_cn, group2infos[grp]['Info'], '-',
                          group2infos[grp]['Reference'], '-\t-', group2infos[grp]['Type_Judge']]
                if skd == 'Viruses':
    ...
    for file in file_total, file_pathogen:
        ...
        tmp_df = tmp_df.astype({'GenusReads': int, 'Reads_Number': int})
        tmp_df = tmp_df.sort_values(by=['Kingdom', 'GenusReads', 'Reads_Number'], ascending=False)
        tmp_df = tmp_df.astype({'GenusReads': str, 'Reads_Number': str})
        tmp_file = file.with_suffix('.tmp')
        with open(tmp_file, 'w') as wfp:
            wfp.write('\t'.join(tmp_df.columns) + '\n')
            for kd in ['Bacteria', 'SpecialPathogens', 'Eukaryota:Fungi', 'Eukaryota:Parasite', 'Viruses']:
                for row in tmp_df.itertuples():
                    if row[1] == kd:
                        wfp.write('\t'.join(row[1:]) + '\n')
        os.system(f'mv {str(tmp_file)} {str(file)}')
