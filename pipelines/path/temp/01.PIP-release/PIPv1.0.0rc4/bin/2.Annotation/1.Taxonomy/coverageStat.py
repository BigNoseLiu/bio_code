...
def get_cov(bam, tid2acc2cpn, kingdom, seqid2taxid, seqtid2info, seqtid2stid, stid2group_tid, df_bacteria):
    ...
    data = [line.split('\t') for line in os.popen(f'{cfg.samtools} view {bam}') if line.strip()]
    if not data:
        return kingdom, None
    ...
    for stid in df_bam_sub['S_tid'].drop_duplicates():
        ...
        for row in df_temp.itertuples():
            if stid in tid2acc2cpn:
                sps_name = seqtid2info[stid]['species'].replace('/', '_')
    ...
    for grp_tid in df_bam_sub['Grp_tid'].drop_duplicates():
        ...
        for row in df_temp.itertuples():
            if grp_tid in tid2acc2cpn:
                sps_name = seqtid2info[grp_tid]['species'].replace('/', '_')
...
def get_cov4sub_virus(bam, tid2acc2cpn, kingdom, seqid2taxid, df_virus, taxid2info):
    ...
    df_virus = df_virus[['tax_id', 'S_tid', 'reads_count', 'species']]
                    ...
                    if bam_row[2] in tid2acc2cpn[row[2]]:
                        for cpn, pos in tid2acc2cpn[row[2]][bam_row[2]].items():
                            ...
                            if pos[0] <= int(bam_row[3]) <= pos[1]:
                                sps2cpn2depth[row[4].replace('/', '_')][cpn] += 1
                                break
        # subspecies level
        else:
...
def stat_cov(indir, db, seqtid2info, seqtid2stid, stid2group_tid, parallel, name, outdir):
    # 获取病毒亚种与种水平taixd关系表
    taxid2info = get_taxid2info(db)
    # 获取病毒检出结果表
    df_virus = pd.read_csv(Path(indir).parent / 'viral.xls', sep='\t', dtype=str)
    if not df_virus.empty:
        df_virus['S_tid'] = df_virus.apply(lambda x: taxid2info['S1_tid2S_tid'].get(x['tax_id'], x['tax_id']), axis=1)

    # 获取细菌检出结果表
    df_bacteria = pd.read_csv(Path(indir).parent / 'bacteria.xls', sep='\t', dtype=str)
...
def draw_cov_plot(dir, script):
    for cov in Path(dir).glob('*cov'):
        name_lst = cov.name.split('.')
        outdir = Path(dir) / f'{name_lst[0]}_readscount_coverage_plot'
        if not outdir.exists():
            os.system(f'mkdir -p {str(outdir)}')
        cmd = f'{cfg.Rscript} {script} {str(cov)} {str(outdir)}'
        os.system(cmd)
...
def main(indir, db, parallel, script, outdir):
    ...
    # 获取PIDB数据库中seq_taxid对应的species_taxid
    ...
    stat_cov(indir, db, seqtid2info, seqtid2stid, stid2group_tid, parallel, name, outdir)
