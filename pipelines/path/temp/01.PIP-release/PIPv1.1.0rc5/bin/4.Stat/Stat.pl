...
my $supplement    = "$Bin/Supplement.py";
...
`python $QC -w $workdir -s $list`;
`$PYTHON3 $Normalize -w $workdir -s $list`;
...
for my $name (keys %samp_list) {
    ...
    if ($analysis =~ /Taxonomy/) {
        ...
        for my $type (keys %{$samp_list{$name}}) {
			...
			`cp -fr $origTaxDir/$name/$type/bacteria.result.fq.gz $sampDir/bacteria.result.fq.gz`; ###temp
			`cp -fr $origTaxDir/$name/$type/fungi.result.fq.gz $sampDir/fungi.result.fq.gz`; ###temp
			`cp -fr $origTaxDir/$name/$type/protozoa.result.fq.gz $sampDir/protozoa.result.fq.gz`; ###temp
			`cp -fr $origTaxDir/$name/$type/viral.result.fq.gz $sampDir/viral.result.fq.gz`; ###temp
			`cp -fr $origTaxDir/$name/$type/LY.result.fq.gz $sampDir/LY.result.fq.gz`; ###temp
...
`$PYTHON3 $supplement -w $workdir -s $list --db  -t 21`;
