...
def get_tax_info(db):
    ...
    for row in df.itertuples():
        if row[5] == 'Viruses':
            ...
            tax_infos['db_code2name'][row[1]] = row[13]
        else:
            tax_infos['taxid2name'][row[3]] = row[11]
            tax_infos['taxid2name'][row[4]] = row[11]
            tax_infos['db_code2name'][row[1]] = row[11]
...
def main(kout, top, db, out, kmercut):
        ...
        for _, line in enumerate(rfp):
            ...
            if kingdom == 'Viruses':
                for mapping in ll[4].split(' '):
                    ...
                    if m[0] in tax_info['taxid2db_code']:
                        name = tax_info['db_code2name'].get(tax_info['taxid2db_code'].get(m[0]))
    ...
    with open(out, 'w') as wfp:
        ...
        for row in df.itertuples():
            if row[8] == 'Viruses':
                db_code = tax_info['taxid2db_code'].get(row[3])
                name = tax_info['db_code2name'].get(db_code)
            else:
