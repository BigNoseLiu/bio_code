...
my($species, $fq, $dbVersion, $num, $outdir);
GetOptions(
	"species:s" => \$species,
	"fq:s" => \$fq,
...
#`$BLASTN -query $outdir/$spe.fna -db $BLASTDB/$dbVersion/nt -out $outdir/$spe.m8 -outfmt "6 qseqid sacc pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen qcovhsp" -num_threads 12 -evalue 0.05 -word_size 28 -reward 1 -penalty -2`;
