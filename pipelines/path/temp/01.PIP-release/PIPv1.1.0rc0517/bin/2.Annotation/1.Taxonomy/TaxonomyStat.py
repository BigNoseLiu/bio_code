...
TXT_HEADER = "Kingdom\tGenusSN\tGenusCN\tGenusReads\tGenusReads(Normalization)\tGenusRelativeAbundance(%)\t" \
             ...
             "SpeciesReads\tSpeciesReads(Normalization)\tSpeciesRelativeAbundance(%)\tGMRN\tTPM\tRH-RPM\t" \
             "Verification\tCoverage\tFocus\tLabel\tType\tDatabaseCode\tFreq/Median/Mean/[Min-Max](Historically)\t" \
             "FoldChange(Historically)\tNCCount\tFoldChange(NC)\tFreq/Median/Mean/[Min-Max](Background)\n"
...
def main(report, mpa, db, name, validate, highconf, type, outdir, version):
    ...
    with open(file_total, 'w') as out_total, open(file_pathogen, 'w') as out_pathogen:
        ...
        for spe, rpm in tax_stat_res['spe2RPM'].items():
            ...
            if spe in spe2anno:
                genus_cn = spe2anno[spe].get('GenusCN', '-')
            else:
                genus_cn = '-'
            ...
            if superkingdom:
                ...
                if group:
                    ...
                    out_total.write("\t-\t-\t0\t0")
                out_total.write(f"\t{spe}\t{spe_cname}\t{spe_lv_rc}\t0\t0\t{genus_max_rc}\t{tpm}\t{rpm}")
                if check_tag:
                    out_total.write(f"\t{check_tag}\t0\t{target_tag}\t-\t-\t-\t0\t0\t0\t0\t-\n")
                else:
                    if score:
                        out_total.write(f"\t{score}\t0\t{target_tag}\t-\t-\t-\t0\t0\t0\t0\t-\n")
                    else:
                        out_total.write(f"\tFalse\t0\t{target_tag}\t-\t-\t-\t0\t0\t0\t0\t-\n")
                ...
                if superkingdom != 'Viruses':
                    if not re.search(' sp\.| genomosp\.| genosp\.', spe):
                        ...
                        out_pathogen.write(f"\t{spe}\t{spe_cname}\t{spe_lv_rc}\t0\t0\t{genus_max_rc}\t{tpm}\t{rpm}")
                        if check_tag:
                            out_pathogen.write(f"\t{check_tag}\t0\t{target_tag}\t-\t-\t-\t0\t0\t0\t0\t-\n")
                        else:
                            if score:
                                out_pathogen.write(f"\t{score}\t0\t{target_tag}\t-\t-\t-\t0\t0\t0\t0\t-\n")
                            else:
        ...
        for grp in group2reads_cnt.keys():
            ...
            if group2infos[grp]['Focus'] == '*':
                ...
                txt_row = [skd, gns, gns_cn, gns_rc, '0', '0', '0',
                           grp, grp_cn, grp_rc, '0',
                           grp, grp_cn, grp_rc, '0', '0', gns_max_rc,
                           "0\t0\t0\t0\t+\t-\t-\t-\t0\t0\t0\t0\t-\n"]
    ...
    for file in file_total, file_pathogen:
        ...
        tmp_df['GenusRelativeAbundance(%)'] = tmp_df.apply(lambda x: x['GenusReads'] / sum_gns_rc * 100, axis=1)
        gns_rc2rank = {}
        ...
        tmp_df['SpeciesRelativeAbundance(%)'] = tmp_df.apply(lambda x: x['SpeciesReads'] / sum_spe_rc * 100, axis=1)
