...
sub task_admin{
	...
	if( $out_files ne "-" ){
		foreach my $name2file( split(/,/,$out_files) ){
			my ($file_type,$file_path) = split(/:/,$name2file);
...
my $bio_dir = "$Bin/../..";
my $bin_dir = "/biodata/common_pipe/bin_v20200825";	#$Bin;
#打印config info
# '#&' 代表特殊意义
# '#&C' 代表全局配置信息


#设置当前运行目录下的data_out为数据输出目录
my $current_dir = `pwd`;
chomp $current_dir;
my $shell_dir = "$current_dir/shells";
$out_dir = "$current_dir/data_out";
`mkdir -p $out_dir $shell_dir`;
#设置docker运行命令
my $docker_out_dir = "/out_dir";
my $docker_opt = "docker run -w $docker_out_dir -u 1002:1002 --rm -v $Bin/../../..:/biodata -v $out_dir:$docker_out_dir";
$out_dir = $docker_out_dir;
my $docker_cmd = "$docker_opt li/biojupyter ";

#软件变量配置
my $bin_mkdir		=	"$docker_cmd mkdir -p";
my $bin_fastp		=	"$docker_cmd fastp";
my $bin_star		=	"$docker_cmd STAR ";
my $star_hg38_index	=	"/biodata/database/refgenome/ncbi/GRCh38/format_main/star_index75";
my $bin_salmon 		=	"$docker_cmd salmon";
my $salmon_hg38_index	=	"/biodata/database/refgenome/ncbi/GRCh38/format_main/salmon_index";
my $bin_suppa		=	"$docker_cmd suppa.py";
my $bin_suppa_select	=	"$docker_cmd multipleFieldSelection.py";
my $hg38_gtf		=	"/biodata/database/refgenome/ncbi/GRCh38/format_main/GCF_000001405.39_GRCh38.p13_genomic.format_main.gtf";
my $hg38_gtf_event	=	"/biodata/database/refgenome/ncbi/GRCh38/format_main/suppa2_event/GCF_000001405.39_GRCh38.p13_genomic.format_main.events.ioe";
...
my $bin_select_snpeff	=	"$docker_cmd perl $bin_dir/snpeff/select_standard_hgvs_from_snpeff_Eff.v1.01.pl";
my $bin_fix_snpeff	=	"$docker_cmd perl $bin_dir/snpeff/fix_snpeff_Eff.v1.pl";
...
my $bin_anno_gene	=	"$docker_cmd perl $bin_dir/var_inter/anno_gene.v2.01.pl";
#HPO:gene to disease
...
my $gatk_dbsnp		=	"/biodata/database/gatk_bundle/20181009/dbsnp_138.b37.vcf";
my $gatk_Mills_and_1000G_gold_standard_indels="/biodata/database/gatk_bundle/20181009/Mills_and_1000G_gold_standard.indels.b37.vcf";
my $gatk_GetPileupSummaries=	"/biodata/database/gatk_bundle/20181009/raw_download/Mutect2/GetPileupSummaries/small_exac_common_3_b37.vcf.gz";
my $mutect_gnomad	=	"/biodata/database/gatk_bundle/20181009/raw_download/Mutect2/af-only-gnomad.raw.sites.b37.vcf.gz";

#global var
my $temp_dir = "$out_dir/temp";
#my $cfg_dir = "$out_dir/cfg";	`mkdir -p $cfg_dir`;
my ( $bed_region, $gatk_interval_list, $gatk_cnv_interval_list );
my $pon4gatk_vcf = "";
...
sub star_aln{
	...
	#任务ID
	my ($print_flag, $sample_id) = @_;
	my $task_id = "$sample_id\_star_aln";
	#初始化文件名
	my $data_dir="$out_dir/star_aln/$sample_id";
	my $star_prefix="$data_dir/$sample_id.star_aln";

	my ($flag_dup, $clean_fq1, $clean_fq2, $pre_task)  = &task_admin($task_id, $sample_id, "clean_fq1,clean_fq2", "star_aln:$star_prefix");
	return if($flag_dup == 1);
	
	#运行脚本
	&cmd_print($print_flag, "#&J	$task_id	$pre_task");
	&cmd_print($print_flag, "$bin_mkdir $data_dir ");
	&cmd_print($print_flag, "$bin_star --twopassMode Basic --quantMode TranscriptomeSAM GeneCounts --runThreadN 6 --alignIntronMin 20 --alignIntronMax 50000 --outSAMtype BAM SortedByCoordinate --sjdbOverhang 74 --outSAMattrRGline ID:sample SM:sample PL:ILLUMINA --outFilterMismatchNmax 2 --outSJfilterReads Unique --outSAMmultNmax 1 --outSAMmapqUnique 60 --readFilesCommand gunzip -c --genomeDir $star_hg38_index --readFilesIn $clean_fq1, $clean_fq2 --outFileNamePrefix $star_prefix");
...
sub suppa2_as{
	...
	my $id = "suppa2_as";
	my ($print_flag, $sample_id) = @_;
	my $task_id = "$sample_id\_$id";
	#初始化文件名
	my $data_dir="$out_dir/$id/$sample_id";
	my $prefix = "abc";
	my $salmon_out ="$data_dir/$sample_id.salmon";
	my $suppa_tpm ="$data_dir/$sample_id.suppa.iso_tpm.txt";
	my $suppa_psi ="$data_dir/$sample_id.suppa.tra2_events";

	my ($flag_dup, $clean_fq1, $clean_fq2, $pre_task)  = &task_admin($task_id, $sample_id, "clean_fq1,clean_fq2", "star_aln:$prefix");
	return if($flag_dup == 1);
	
	#运行脚本
	&cmd_print($print_flag, "#&J	$task_id	$pre_task");
	&cmd_print($print_flag, "$bin_mkdir $data_dir ");
	&cmd_print($print_flag, "$bin_salmon quant -i $salmon_hg38_index -l ISF --gcBias -1 $clean_fq1 -2 $clean_fq2 -p 4 -o $salmon_out");
	&cmd_print($print_flag, "$bin_suppa_select -k 1 -f 4 -o $suppa_tpm -i $salmon_out/quant.sf");
	&cmd_print($print_flag, "$bin_suppa psiPerEvent -i $hg38_gtf_event -e $suppa_tpm -o $suppa_psi");
...
sub gatk_preprocess{
	...
	my ($flag_dup, $in_fq1, $in_fq2, $pre_task)  = &task_admin($task_id, $sample_id, "clean_fq1,clean_fq2", "recal_bam:$recal_bam,sorted_bam:$sorted_bam,table_getpileupsummaries:$table_getpileupsummaries");
	return if($flag_dup == 1);
...
sub gatk_qc{
	...
	#set names
	my $data_dir="$out_dir/QC/$sample_id";
	my $qc_prefix="$data_dir/$sample_id.picardQC";

	my ($flag_dup, $sorted_bam, $pre_task )  = &task_admin($task_id, $sample_id, "sorted_bam", "-");
	return if($flag_dup == 1);
...
sub germline_gatk4{
	...
	my ($sample_id) = @_;
	if( defined( $h_cmd_log{"germline_gatk4"}{$sample_id} ) ){
		return;
	}
	else{
		$h_cmd_log{"germline_gatk4"}{$sample_id} = 1;
	}
	...
	#set names
	my $data_dir="$out_dir/germline_gatk4/$sample_id";     `mkdir -p $data_dir`;
	my $germline_raw_vcf="$data_dir/germline.$sample_id\.raw.vcf.gz";#	$h_sample_info{"sample"}{$sample_id}{"germline_raw_vcf"} = $germline_raw_vcf;
	my $germline_filter_vcf="$data_dir/germline.$sample_id\.filter.vcf";#	$h_sample_info{"sample"}{$sample_id}{"germline_filter_vcf"} = $germline_filter_vcf;
	$h_germline_samples{$sample_id}++;
	
	#germline variant call with GATK4
	my $last_job = "$sample_id\_aln";
	&cmd_print("#&J	$sample_id\_CallVariantGermlineGATK4	$last_job");
	my $t_interval = "";
	$t_interval = "-L $gatk_interval_list" if( defined($gatk_interval_list) && $gatk_interval_list =~ /\S/ );
	&cmd_print("$bin_gatk HaplotypeCaller -R $ref_fa -I $recal_bam $t_interval -O $germline_raw_vcf");
	#Hard filter following:https://software.broadinstitute.org/gatk/documentation/article?id=11069
	&cmd_print("$bin_gatk VariantFiltration -R $ref_fa -V $germline_raw_vcf -O $germline_filter_vcf --filter-name \"GATKHardFilter\" --filter-expression \"(QD < 2.0) || (FS > 60.0) || (MQ < 40.0) || (MQRankSum < -12.5) || (ReadPosRankSum < -8.0) || (SOR > 3.0)\"");
	&cmd_print("$bin_gatk SelectVariants -R $ref_fa -V $germline_filter_vcf -O $germline_filter_vcf.exclude-non-variants.vcf --exclude-non-variants");
	...
	$h_sample_info{"call_variant"}{"GermlineGATK4"}{$sample_id}{"final_vcf"} = "$germline_filter_vcf.exclude-non-variants.vcf";
...
sub anno_germline_variant{
	...
	my ($sample_id) = @_;
	if( defined( $h_cmd_log{"anno_germline_variant"}{$sample_id} ) ){
		return;
	}
	else{
		$h_cmd_log{"anno_germline_variant"}{$sample_id} = 1;
	}

	my $last_job = "$sample_id\_CallVariantGermlineGATK4";

	#get input
	my $final_vcf = $h_sample_info{"call_variant"}{"GermlineGATK4"}{$sample_id}{"final_vcf"};
	#set names
	my $data_dir="$out_dir/germline_anno";	`mkdir -p $data_dir`;
	...
	#注释基因信息by snpeff
	&cmd_print("#&J	$sample_id\_AnnoVariant	$last_job");
...
sub QC_stat{
	...
	if( defined( $h_cmd_log{"QC_stat"} ) ){
		print STDERR "ERR:only one QC_stat allowed\n";
		return;
	}
	else{
		$h_cmd_log{"QC_stat"} = 1;
	...
	my $input_str = "";
	foreach my $sample_id( keys(%{$h_sample_info{"sample"}}) ){
		if( defined($h_sample_info{"sample"}{$sample_id}{"QC"}) ){
			foreach my $qc_metrics( keys(%{$h_sample_info{"sample"}{$sample_id}{"QC"}}) ){
				$last_job .= ",$sample_id\_QC";
				$input_str .= " $sample_id:$qc_metrics";
			}
		}
	}
	if($last_job =~ s/^,//){
		my $data_dir = "$out_dir/final_result";	`mkdir -p $data_dir`;
		&cmd_print("#&J	AllStatQC	$last_job");
		#&cmd_print("$bin_statQC $data_dir/QC_stat.xls $input_str");
		&cmd_print("$bin_statPicardQC $data_dir/AllStat.QC_stat.xls $input_str");
	}
}
