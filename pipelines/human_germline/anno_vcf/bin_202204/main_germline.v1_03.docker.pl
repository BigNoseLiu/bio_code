...
my $bin_create_exomiser_exome=	"cat $bin_dir/exomiser/exome.yml|perl $bin_dir/exomiser/create_yml.v2.01.pl";
#annovar
...
chomp @lines;
#&germline_pip(@lines);
my %h_all_sample_result = ();
my $proband = "";
my $ped_file = "";
my $hpo_ids = "-";
my $analysis_id = "no_ids";
foreach my $line( @lines ){
	#获取全局变量
	if($line =~ /^#/){
		if( $line =~ /^#proband\t(\S+)\s*$/ ){
			$proband = "liumm_proband:$1";
		}
		if( $line =~ /^#ped\t(\S+)\s*$/ ){
			my $t_ped_file = "/mnt/$1";
			$ped_file = "liumm_ped_file_in:$t_ped_file";
		}
		if( $line =~ /^#hpo\t(\S+)\s*$/ ){
			$hpo_ids = "liumm_hpo_ids:$1";
		}
		if( $line =~ /^#id\t(\S+)\s*$/ ){
			$analysis_id = $1;
		}
...
close IN;
my $merge_cmd = "";
foreach my $sample_id( sort {$a cmp $b} keys(%h_all_sample_result) ){
	$merge_cmd .= " ".$h_all_sample_result{$sample_id}{'vcf'};
}
my $is_merge_flag = 1;
if($is_merge_flag == 1){
	my $sample_id = "merge_all";
	my $data_dir="$docker_out_dir/merge_result";
	my $merge_vcf_file = "$data_dir/$sample_id.vcf";
	my $merge_fix_vcf_file = "$data_dir/$sample_id.exomiser_fix.vcf";
	my $ped_fix_file = "$data_dir/$sample_id.fix.ped";
	...
	if( $hpo_ids ne "-" ){
		&docker_print("$bin_create_exomiser_exome $proband $ped_file liumm_ped_file_path:$ped_fix_file liumm_vcf_file_in:$merge_vcf_file liumm_vcf_file_path:$merge_fix_vcf_file liumm_output_prefix:$exomiser_anno_prefix $hpo_ids >$yml_exomiser_file");
	...
	&docker_print("$bin_snpeff -s $snpeff_anno_stat $merge_vcf_file >$snpeff_anno_vcf");
