...
foreach $line(@lines  ){
	...
	if( $line =~ /^#/ ){
		if( $flag_inter == 0 && $line =~ /^##FILTER/  ){
			$flag_inter = 1;
			print OUT '##FILTER=<ID=NearbyVariant,Description="close to other variant(<'.$min_dis.'bp)">'."\n";
			print OUT '##FILTER=<ID=OverlapVariant,Description="overlap with other variant">'."\n";
		}
		elsif( $line =~ /^#CHROM/  ){
...
sub usageWithColor {
	...
	print <<USAGE;
Description:
        Used to 1. tag nearby($min_dis bp) & overlap variant. 2. fix asterisk(*) in vcf4.2 for vcf generated by gatk4
Usage:  
