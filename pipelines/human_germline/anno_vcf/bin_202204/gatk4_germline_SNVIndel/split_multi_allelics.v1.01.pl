	...
	if($arr[4] =~ /,/){
		...
		for( my$i=0;$i<scalar@alt_arr;$i++ ){
			...
			for( my$m=9;$m<scalar@arr;$m++ ){
				...
				#优先处理GT列，并初始化./.的其他列为.
				if( $gt_i != 10000 && $format_value_arr[$gt_i] =~ /^\d+\/\d+$/ ){
					#修改当前样本的GT的内容
					my @t_arr_geno  = split(/\//,$format_value_arr[$gt_i]);
					if($t_arr_geno[0] eq $x && $t_arr_geno[1] eq $x){
						$format_value_arr[$gt_i] = "1/1";
					}
					elsif($t_arr_geno[0] eq $x || $t_arr_geno[1] eq $x){
