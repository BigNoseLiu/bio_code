...
my @headers = ("TOTAL_READS","READ_LENGTH","TOTAL_BASES","PCT_PF_READS_ALIGNED","Q20_BASES","Q30_BASES","MEDIAN_INSERT_SIZE","PERCENT_DUPLICATION","TARGET_TERRITORY","PCT_SELECTED_BASES","MEAN_BAIT_COVERAGE",'MEAN_TARGET_COVERAGE',"PCT_TARGET_BASES_1X","PCT_TARGET_BASES_2X","PCT_TARGET_BASES_10X","PCT_TARGET_BASES_20X","PCT_TARGET_BASES_30X","PCT_TARGET_BASES_100X",'PCT_TARGET_BASES_250X','PCT_TARGET_BASES_500X','PCT_TARGET_BASES_1000X','PCT_TARGET_BASES_2500X','PCT_TARGET_BASES_5000X');
my @sample_ids  = sort {$a cmp $b} keys(%h_sample);
open OUT,">$ARGV[0]" or die $!;
my $out_header = join("\t",@headers);
$out_header =~ s/SELECTED_BASES/CaptureRatio/g;
$out_header =~ s/MEAN_BAIT_COVERAGE/Raw_coverage/g;
$out_header =~ s/MEAN_TARGET_COVERAGE/Effective_coverage/g;
$out_header =~ s/PCT_PF_/\(%\)/g;
$out_header =~ s/PCT_/\(%\)/g;
$out_header =~ s/PERCENT_/\(%\)/g;
$out_header =~ s/Q20_BASES/\(%\)Q20/g;
...
foreach my $sample_id( @sample_ids ){
	...
	foreach my $header( @headers ){
		if( defined($h_stat{$header}{$sample_id}) ){
			...
			if( $header eq "Q20_BASES" || $header eq "Q30_BASES" ){
